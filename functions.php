...
//文末文章分页显示
function content_pagesNav($content)
{
  /* 使用到的函数
  ...
  if($multipage && ( is_single() || is_page() ) )//只有在多页的文章或页面中才会显示导航栏
  {
    ...
    for ($i = 1; $i <= $numpages; $i++) {
      	...
      	if ($i != $page || !$more && 1 == $page) {
		  	//通过正则表达式为其添加class来匹配主题的样式
			$pattern_temp = '/class="post-page-numbers"/i';
			$replacement_temp = 'class="post-page-numbers page-link"';
			$pregRet = preg_replace($pattern_temp, $replacement_temp, _wp_link_page($i));
        	$link = '<li class="page-item">' . $pregRet . $link .'</a></li>';
      	} elseif ($i === $page) {
        	$link = '<li class="page-item active"><span class="page-link" href="#">' . $link. '</span></li>';     
      	}
    	$content .= $link;
    }
...
function shortcode_collapse_block($attr,$content=""){
	...
	$title = isset($attr['title']) ? $attr['title'] : 'untitled';
	switch ($color){
		case 'indigo':
